...
const CarCard = ({ car }) => {
  ...
  return (
    <div className="relative bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col">
      {/* Tag Badge */}
      {car.tag && (
        <div
          className={`absolute top-4 left-4 ${tagColor} text-white text-xs font-bold px-3 py-1 rounded-full shadow`}
        >
          {car.tag}
        </div>
      )}
      ...
      {/* Image Gallery */}
      <div className="rounded-t-xl overflow-hidden border-b border-gray-200">
        <ImageGallery
          items={galleryItems}
          showPlayButton={false}
          showFullscreenButton={true}
