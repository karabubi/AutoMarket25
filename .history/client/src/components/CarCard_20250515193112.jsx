  ...
  return (
    <div className="flex flex-col md:flex-row bg-white rounded-2xl shadow-md hover:shadow-lg transition-all duration-300 overflow-hidden max-w-6xl mx-auto">
      ...
      <div className="w-full md:w-1/2 flex flex-col justify-between p-6 bg-gray-50 space-y-4">
        ...
        {/* Title & Description */}
        <div>
          <h2 className="text-2xl font-bold text-gray-800 mb-1">{car.make} {car.model}</h2>
          <p className="text-sm text-gray-600 mb-2">{car.description || 'Keine Beschreibung vorhanden'}</p>
          <p className="text-sm text-gray-500">
            📍{' '}
            <a
              href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(displayLocation)}`}
        ...
        <div className="bg-white border border-gray-300 rounded-xl p-4 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <p className="text-xl font-bold text-green-700">€{car.price.toLocaleString()}</p>
              <p className="text-xs text-gray-500">inkl. MwSt.</p>
            </div>
        ...
        {/* Dealer Info */}
        <div className="flex items-center gap-4 bg-white border border-gray-300 rounded-xl p-4 shadow-sm">
          <img
            src={dealer.logoUrl}
            alt={`${dealer.name} logo`}
            className="w-14 h-14 rounded-full object-cover border"
