...
// ✅ CarCard.jsx (only displays images, not uploads)
import { Link } from 'react-router-dom';
import ImageGallery from 'react-image-gallery';
import 'react-image-gallery/styles/css/image-gallery.css';

const CarCard = ({ car }) => {
  const galleryItems = (car.images?.length ? car.images : car.image_url ? [{ image_url: car.image_url }] : []).map((img) => ({
    original: img.image_url,
    thumbnail: img.image_url,
  }));
  ...
  const displayLocation = car.location || 'Honnefer Str. 19, 53179 Bonn';
    ...
    <div className="flex flex-col md:flex-row bg-white rounded-2xl shadow-md hover:shadow-lg transition-all duration-300 overflow-hidden max-w-6xl mx-auto">
      ...
      <div className="w-full md:w-1/2 flex flex-col justify-between p-6 bg-gray-50 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
          ...
          <div className="flex gap-3 mt-3 sm:mt-0">
            <Link to={`/car/${car.id}`} className="bg-orange-600 text-white px-4 py-2 rounded hover:bg-orange-700 text-sm font-medium">
              Details
            </Link>
            <a href={`tel:${dealer.phone}`} className="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 text-sm font-medium">
              Call Now
            </a>
          </div>
        </div>
