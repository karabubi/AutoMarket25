...
const Settings = () => {
  ...
  const [confirmDelete, setConfirmDelete] = useState({ show: false, target: null });
  ...
  const handleDelete = async () => {
    if (!user || user.role !== 'admin') {
      setInfoMessage('Unauthorized action');
      setConfirmDelete({ show: false, target: null });
      return;
    }
    ...
    try {
      switch (confirmDelete.target) {
        case 'all-cars':
          await deleteAllCars(token);
          setCars([]);
          setInfoMessage('All cars deleted successfully');
          break;
        case 'all-payments':
          await deleteAllPayments(token);
          setInfoMessage('All payments deleted successfully');
          break;
        default:
          if (confirmDelete.target.startsWith('payment:')) {
            const id = confirmDelete.target.split(':')[1];
            await deletePayment(id, token);
            setInfoMessage(`Payment ${id} deleted`);
          } else {
            await deleteCar(confirmDelete.target, token);
            setCars(cars.filter(car => car.id !== confirmDelete.target));
            setInfoMessage('Car deleted successfully');
          }
      }
    } catch (err) {
      setInfoMessage(err.response?.data?.message || 'Error during deletion');
    } finally {
      setConfirmDelete({ show: false, target: null });
  ...
  return (
    <div className="settings-container">
      ...
      <button onClick={() => setConfirmDelete({ show: true, target: 'all-cars' })}>Delete All Cars</button>
      {cars.map(car => (
        <div key={car.id}>
          <span>{car.make} {car.model}</span>
          <Link to={`/edit/${car.id}`}>Edit</Link>
          <button onClick={() => setConfirmDelete({ show: true, target: car.id })}>Delete</button>
        </div>
      ))}
      <button onClick={() => setConfirmDelete({ show: true, target: 'all-payments' })}>Delete All Payments</button>
      <input type="text" value={paymentId} onChange={e => setPaymentId(e.target.value)} placeholder="Payment ID" />
      <button onClick={() => setConfirmDelete({ show: true, target: `payment:${paymentId}` })}>Delete Payment</button>

      {confirmDelete.show && (
        <div className="modal">
          <p>Are you sure you want to delete?</p>
          <button onClick={handleDelete}>Confirm</button>
          <button onClick={() => setConfirmDelete({ show: false, target: null })}>Cancel</button>
        </div>
      )}
    </div>
  );
