...
// // Enhanced Settings.jsx with Admin Deletion Controls
// import { useEffect, useState } from 'react';
// import { fetchAllCarsAdmin, deleteCar, deleteAllCars, deletePayment, deleteAllPayments } from '../../utils/api';
// import { Link } from 'react-router-dom';
// import { useAuth } from '../../context/AuthContext';
...
//       if (err.response?.status === 403) setInfoMessage('❌ You do not have permission to delete');
//       else if (err.response?.status === 401) setInfoMessage('❌ Session expired. Please log in again');
//       else setInfoMessage(`❌ ${message}`);
//     }
//     setTimeout(() => setInfoMessage(''), 3000);
//   };
...
//       ) : cars.length === 0 ? (
//         <div className="bg-gray-50 rounded-xl p-8 text-center">No car listings found</div>
//       ) : (
//         <div className="space-y-6">
//           {cars.map(car => (
//             <div key={car.id} className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-200 flex justify-between">
//               <div className="p-5 flex-1">
//                 <h3 className="font-bold text-xl text-gray-900">{car.make} {car.model}</h3>
//                 <p className="text-gray-500 mt-1">€{Number(car.price).toLocaleString()} • {car.year}</p>
//                 {car.user_name && (
//                   <p className="mt-2 text-sm text-gray-500 italic">Owner: {car.user_name}</p>
//                 )}
//                 <div className="flex justify-end gap-4 mt-4">
//                   <Link to={`/edit/${car.id}`} className="text-blue-600 hover:text-blue-800 text-sm">Edit</Link>
//                   {isAdmin && (
//                     <button onClick={() => confirmDelete(car.id)} className="text-red-600 hover:text-red-800 text-sm">Delete</button>
//                   )}
//                 </div>
//               </div>
//               <div className="p-4">
//                 <img src={car.image_url || '/default-car.jpg'} alt="car" className="w-24 h-24 object-cover rounded-lg" />
//               </div>
//             </div>
//           ))}
//         </div>
...
//               value={paymentIdInput}
//               onChange={(e) => setPaymentIdInput(e.target.value)}
//               className="flex-1 rounded-l-lg border px-3 py-2 focus:ring-blue-500"
//               placeholder="Enter payment ID"
//             />
//             <button
//               type="button"
//               onClick={() => confirmDelete('payment:' + paymentIdInput)}
//               className="rounded-r-lg bg-red-600 text-white px-4 py-2 font-medium hover:bg-red-700 disabled:bg-gray-400"
//               disabled={!paymentIdInput}
//             >
//               Delete Payment
//             </button>
...
//                  deleteTarget === 'all-payments' ? 'Delete ALL payments?' :
//                  deleteTarget.startsWith('payment:') ? 'Delete this payment?' : 'Delete this car?'}
//               </p>
//               <div className="flex gap-3">
//                 <button onClick={handleCancel} className="flex-1 py-3 px-4 rounded-lg border text-gray-700 hover:bg-gray-50">Cancel</button>
//                 <button onClick={handleDeleteConfirmed} className="flex-1 py-3 px-4 rounded-lg bg-red-600 text-white hover:bg-red-700">
//                   Confirm
//                 </button>
//               </div>
//             </div>
