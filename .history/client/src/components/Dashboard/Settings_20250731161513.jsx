...
// Import necessary contexts or components
import { AuthContext } from '../context/AuthContext';  // assume AuthContext provides current user
import ConfirmModal from './ConfirmModal';             // component for confirmation dialogs

function Settings() {
    const { user } = useContext(AuthContext);
    const isAdmin = user?.role === 'admin';  // [Added] Only allow delete features for admin
    
    // State for confirmation modals and input fields
    const [confirmDeleteAllCarsOpen, setConfirmDeleteAllCarsOpen] = useState(false);
    const [confirmDeleteCarId, setConfirmDeleteCarId] = useState(null);
    const [confirmDeleteAllPaymentsOpen, setConfirmDeleteAllPaymentsOpen] = useState(false);
    const [deletePaymentId, setDeletePaymentId] = useState('');
    const [confirmDeletePaymentIdOpen, setConfirmDeletePaymentIdOpen] = useState(false);
    
    const [cars] = useState([]);
    
    useEffect(() => {
        // Preserve existing data-fetching logic (if any)
        // e.g., fetch all cars and payments
    }, []);
    
    // Delete All Cars handler (opens confirmation modal)
    const handleDeleteAllCarsClick = () => {
        if (!isAdmin) return;  // [Added] Prevent non-admin from triggering delete
        setConfirmDeleteAllCarsOpen(true);
    };
    
    // Confirm deletion of all cars
    const confirmDeleteAllCars = () => {
        // Existing deletion logic for all cars
        // ...
        setConfirmDeleteAllCarsOpen(false);
    };
    
    // Delete single car handler (opens confirmation modal)
    const handleDeleteCarClick = (carId) => {
        if (!isAdmin) return;  // [Added] Prevent non-admin from triggering delete
        setConfirmDeleteCarId(carId);
    };
    
    // Confirm deletion of a single car
    const confirmDeleteCar = (carId) => {
        // Existing deletion logic for carId
        // ...
        setConfirmDeleteCarId(null);
    };
    
    // Delete All Payments handler (opens confirmation modal)
    const handleDeleteAllPaymentsClick = () => {
        if (!isAdmin) return;  // [Added] Prevent non-admin from triggering delete
        setConfirmDeleteAllPaymentsOpen(true);
    };
    
    // Confirm deletion of all payments
    const confirmDeleteAllPayments = () => {
        // Existing deletion logic for all payments
        // ...
        setConfirmDeleteAllPaymentsOpen(false);
    };
    
    // Delete Payment by ID handler (opens confirmation modal or triggers deletion)
    const handleDeletePaymentByIdClick = () => {
        if (!isAdmin) return;  // [Added] Prevent non-admin from triggering delete
        if (deletePaymentId.trim() === '') return;  // no ID entered
        setConfirmDeletePaymentIdOpen(true);
    };
    
    // Confirm deletion of a specific payment by ID
    const confirmDeletePaymentById = () => {
        // Existing deletion logic for payment with id = deletePaymentId
        // ...
        setConfirmDeletePaymentIdOpen(false);
        setDeletePaymentId('');
    };
    ...
    return (
        <div className="settings-page">
            ...
            <section className="all-cars-section">
                <h2>All Cars</h2>
                <p>Manage all vehicle listings in the system.</p>
                {/* "Delete All Cars" button (disabled for non-admin) [Added] */}
                <button 
                    onClick={handleDeleteAllCarsClick} 
                    disabled={!isAdmin} 
                ...
                {/* List of cars with individual edit/delete actions */}
                <ul className="cars-list">
                    {cars.map(car => (
                        <li key={car.id} className="car-item">
                            <span>{car.make} {car.model} (Owner: {car.owner})</span>
                            ...
                            {/* Delete button (disabled for non-admin) [Added] */}
                            <button 
            ...
            <section className="all-payments-section">
                <h2>All Payments</h2>
                <p>Manage all payments in the system.</p>
                {/* "Delete All Payments" button (disabled for non-admin) [Added] */}
                <button 
                    onClick={handleDeleteAllPaymentsClick} 
                    disabled={!isAdmin} 
                    title={!isAdmin ? 'Admin only' : undefined} 
                    className="btn btn-danger"
                >
                ...
                {/* "Delete Payment by ID" input and button (disabled for non-admin) [Added] */}
                <div className="delete-by-id">
                    ...
                    />
                    <button 
                        onClick={handleDeletePaymentByIdClick} 
                        disabled={!isAdmin || deletePaymentId.trim() === ''} 
                        title={!isAdmin ? 'Admin only' : undefined} 
                        className="btn btn-danger"
                    >
                        Delete Payment by ID
            ...
            {/* Confirmation Modals (only render if user is admin) */}
            {confirmDeleteAllCarsOpen && isAdmin && (
                <ConfirmModal 
                    title="Confirm Delete All Cars" 
                    message="Are you sure you want to delete all car listings?" 
                    onConfirm={confirmDeleteAllCars} 
                    onCancel={() => setConfirmDeleteAllCarsOpen(false)} 
                />
            )}
            {confirmDeleteCarId !== null && isAdmin && (
                <ConfirmModal 
                    title="Confirm Delete Car" 
                    message={`Are you sure you want to delete car ID ${confirmDeleteCarId}?`} 
                    onConfirm={() => confirmDeleteCar(confirmDeleteCarId)} 
                    onCancel={() => setConfirmDeleteCarId(null)} 
                />
            )}
            {confirmDeleteAllPaymentsOpen && isAdmin && (
                <ConfirmModal 
                    title="Confirm Delete All Payments" 
                    message="Are you sure you want to delete all payments?" 
                    onConfirm={confirmDeleteAllPayments} 
                    onCancel={() => setConfirmDeleteAllPaymentsOpen(false)} 
                />
            )}
            {confirmDeletePaymentIdOpen && isAdmin && (
                <ConfirmModal 
                    title="Confirm Delete Payment" 
                    message={`Are you sure you want to delete payment ID ${deletePaymentId}?`} 
                    onConfirm={confirmDeletePaymentById} 
                    onCancel={() => setConfirmDeletePaymentIdOpen(false)} 
                />
            )}
        </div>
    );
