...
function Settings() {
  ...
  useEffect(() => {
    // Hier kannst du z.B. fetchAllCarsAdmin(token) aufrufen
    // Beispiel:
    // fetchAllCarsAdmin().then((res) => setCars(res.data));
  }, []);
  ...
  return (
    <div className="settings-page p-6">
      ...
      <section className="mb-10">
        ...
        <p className="text-gray-600 mb-3">Manage all vehicle listings in the system.</p>
        <button
          onClick={handleDeleteAllCarsClick}
          disabled={!isAdmin}
          title={!isAdmin ? 'Admin only' : ''}
          className={`btn btn-danger ${!isAdmin ? 'opacity-50 cursor-not-allowed' : ''}`}
        >
          Delete All Cars
        </button>
        <ul className="mt-4 space-y-2">
          {cars.map((car) => (
            <li key={car.id} className="flex justify-between items-center border p-3 rounded-lg">
              <div>
                <strong>{car.make} {car.model}</strong> — {car.owner}
              </div>
      ...
      <section>
        ...
        <p className="text-gray-600 mb-3">Manage all payments in the system.</p>
        <button
        ...
        <div className="mt-4 flex gap-2 items-center">
          <input
            ...
            disabled={!isAdmin}
            title={!isAdmin ? 'Admin only' : ''}
            className={`border rounded px-3 py-2 ${!isAdmin ? 'bg-gray-100' : ''}`}
          />
          <button
