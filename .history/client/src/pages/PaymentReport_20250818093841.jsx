...
//Users/salehalkarabubi/works/27-05-2025 AutoMarket25/AutoMarket25/client/src/pages/PaymentReport.jsx
import React, { useEffect, useState } from 'react';
import axios from 'axios';
import { Bar } from 'react-chartjs-2';
import {
...
const PaymentReport = () => {
  ...
  const totals = payments.reduce(
    (acc, p) => {
      acc.paid += parseFloat(p.paid_amount) || 0;
      acc.unpaid += parseFloat(p.not_paid_amount) || 0;
      return acc;
    },
    { paid: 0, unpaid: 0 }
  );
  ...
  const barData = {
    ...
    datasets: [
      {
        label: 'Paid Amount (€)',
        data: payments.map((p) => parseFloat(p.paid_amount)),
        backgroundColor: 'rgba(34,197,94,0.6)',
      },
      {
        label: 'Unpaid Amount (€)',
        data: payments.map((p) => parseFloat(p.not_paid_amount)),
        backgroundColor: 'rgba(239,68,68,0.6)',
      },
    ],
  };
          ...
          <tbody>
            {payments.map((p) => (
              <tr key={p.id} className="border-t hover:bg-gray-50">
                <td className="px-4 py-2">{p.id}</td>
                <td className="px-4 py-2">#{p.car_id}</td>
                <td className="px-4 py-2 text-right">€{parseFloat(p.paid_amount).toFixed(2)}</td>
                <td className="px-4 py-2 text-right">€{parseFloat(p.not_paid_amount).toFixed(2)}</td>
                <td className="px-4 py-2 text-right">
                  {new Date(p.created_at).toLocaleDateString('de-DE')}<br />
                  <span className="text-xs text-gray-500">
                    {new Date(p.created_at).toLocaleTimeString('de-DE')}
