...
/**
 * Always call "/api" from the frontend.
 * - Local dev: Vite proxy handles "/api" -> localhost backend
 * - Production (Vercel): vercel.json rewrites "/api/*" -> Render backend
 */
const API = axios.create({
  baseURL: "/api",
...
export const deletePayment = (paymentId, token) =>
  API.delete(`/payments/${paymentId}`, authHeader(token));
export const deleteAllPayments = (token) => API.delete("/payments/admin/wipe", authHeader(token));
