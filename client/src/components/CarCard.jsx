...
const CarCard = ({ car }) => {
  ...
  /* -------------------------
     Dealer (fallback-safe)
  -------------------------- */
  const dealer = car.dealer || {
    name: "AutoMarket25",
    phone: "004917655105979",
    email: "karabubi66@yahoo.com",
    logoUrl: logo, // ✅ USE IMPORTED LOGO
  };

  const fallbackLocation = "Honnefer Str., 53179 Bonn";
    ...
    <div className="w-full max-w-6xl mx-auto bg-white rounded-2xl shadow-md hover:shadow-lg transition-all duration-300 overflow-hidden mb-10">
      <div className="flex flex-col md:flex-row">
        ...
        <div className="w-full md:w-1/2 flex flex-col justify-between p-6 bg-gray-50 space-y-6">
          ...
          {car.tag && (
            <span
              className={`self-start px-3 py-1 text-xs font-bold text-white rounded-full ${tagColor}`}
            >
              {car.tag}
            </span>
          )}
